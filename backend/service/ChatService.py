...
class ChatService:
    ...
    async def chat(self, human_input: str, session_id: int, clear_history: bool = False) -> Dict[str, Any]:
        """
        执行聊天操作
    ...
    async def get_conversation_history(self, session_id: int) -> ChatHistoryResponse:
        """
        获取会话的对话历史
        :param session_id: 会话ID
        :return: 对话历史响应对象
    ...
    async def clear_conversation_history(self, session_id: int):
        ...
        try:
            ...
            await clear_conversation_history(thread_id)
