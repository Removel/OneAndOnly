...
class AsyncChatManager:
    ...
    async def get_checkpointer(self):
        if self._checkpointer is None:
            db_path = Path(__file__).parent.parent / "database" / "agent" /"checkpoints_async.db"
            db_path.parent.mkdir(parents=True, exist_ok=True)
...
# 异步聊天管理类实体（用于流式聊天）
_async_chat_manager = AsyncChatManager()
...
async def chat_stream_with_agent(
    ...
    try:
        if clear_history:
            await _async_chat_manager.clear_history(session_id)
        
        compiled_graph = await _async_chat_manager.get_compiled_graph()
