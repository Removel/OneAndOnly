...
### 5. test_chat_history_response.py
聊天历史响应类单元测试文件，测试新增的响应类功能。

**测试内容：**
- MessageResponse 类的角色映射功能
- ChatHistoryResponse 类的数据封装功能
- 空历史记录的处理能力
- LangChain 消息类型到前端角色的正确转换

**运行方式：**
```bash
python test_scripts/backend_test/test_chat_history_response.py
```

**测试覆盖：**
- ✓ HumanMessage → user 角色映射
- ✓ AIMessage → assistant 角色映射
- ✓ SystemMessage → system 角色映射
- ✓ 未知消息类型 → unknown 角色映射
- ✓ 消息字段正确转换（content, message_id, additional_kwargs, response_metadata）
- ✓ ChatHistoryResponse 的 session_id、messages、total_count 字段
- ✓ 空消息列表的处理
